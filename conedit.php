...
$data1 = $madmin->econ($contid);
$a = $data1['title'];
$p = $data1['pic'];
$b = $data1['content'];
$c = $data1['category'];
$co = str_replace("images", "../images", $data1['content']);
}
...
$cont = $_POST['content'];
$cat = $_POST['cat'];
$cont = str_replace("../images", "images", $cont); 
$data = $madmin->editc($id, $tit, $pic, $cont, $cat);
if($data){
	$us = $ud['enteru'];
	$cid = $data;
	$ac = "Edited  Content With ID-"  .$cid;
	$ips = preg_replace('#[^0-9.]#', '', getenv('REMOTE_ADDR'));
	$logs = $madmin->logs($us, $ac, $ips);
header("Location: vcon.php?page");
}else{
header("Location: conedit.php");
}
}
?>
<!DOCTYPE html>
